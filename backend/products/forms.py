...
class CategoryForm(BootstrapForm, forms.ModelForm):
    children = forms.ModelMultipleChoiceField(widget=CustomSelectMultipleWithUrl(ajax_url='/products/categories/sb/'),required=False,queryset=Category.objects.none())
    class Meta:
        model = Category
        fields = ('name', 'image', 'is_published', 'children')
